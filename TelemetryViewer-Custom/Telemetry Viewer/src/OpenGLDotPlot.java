...
//		float speed = torque * rpm / 63025;
		...
		if (haveDatasets) {
			try {
				for (int datasetN = datasetsCount - sampleCount; datasetN < datasetsCount; datasetN++) {
					try {
						float[] color;
						if (datasetN == datasetsCount - 1)
							color = new float[] { 1f, 0f, 1f, 1f};
						else
							color = new float[] { 1f, 0f, 0f, speed.get(datasetN) / topSpeed };
						...
						OpenGL.drawQuad2D(gl, new float[] { 0f, 1f, 0f, 1f }, realX, realY, realX + 20f, realY + 20f);
					}
				}
			} catch (Exception e) {
				for (int datasetN = 0; datasetN < datasetsCount; datasetN++) {
					try {
						...
						OpenGL.drawQuad2D(gl, new float[] { 0f, 0f, 1f, 1f }, realX, realY, realX + 20f, realY + 20f);
					}
				}
			}
		}
